...
|---|---|---|---|---|
| 9 | **Voodoo** (bright purple) | **Soul Ignition** (18 s): a long self-empowerment window (~10 s) — much faster run, bigger jump/dash/slide-jump; touching an enemy knocks them back and slows their whole kit 3 s (reset, never stacked) | **Phantom**: ignite completely — the fastest run and dash in the game, the **jump deliberately untouched** — and phase through bodies, with an inverted skin (mask included); passing through an enemy stuns 2 s. Falling through a head is still a stomp | Masked spirit brawler, purple head-flames |
...
- **A sleep ends three ways only: it expires, a stun lands, or a fresh debuff lands.** Collecting on the setup is what finishes it — a stomp (whose stun wakes them), a bolt, a slash. Nothing *else* breaks it: terrain launches in particular do not — a sleeper who wanders onto a jump spring is thrown, asleep, and wakes wherever the timer says.
