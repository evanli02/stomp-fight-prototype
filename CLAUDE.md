...
1. **Only stomps remove lives.** No ability, ultimate, hazard, or fall may ever remove a life. If you find code that lets anything else deal a "kill," it is a bug.
   ...
   - *Two second-wave designs touch the same territory and are specified, not bugs:* Voodoo's Phantom (falling through a head while phasing is still a stomp, via `receive_stomp`) and Saint's Benediction ward (a stomp on a blessed player consumes the blessing instead of the life, as an early-out **inside** `receive_stomp`, like grace). Both live in `docs/NEW_HEROES.md`; neither creates a new life-touching path.
...
4. **One ultimate per HERO per round** — three per player across a trio, each spent against the hero who cast it — with a ~10 s gap enforced per *player* between any two (`MatchState.ults_used` / `ULT_COOLDOWN`). Swapping hands you a fresh hero's ultimate; it never skips the gap. *Changed from two-per-player by the project owner on 2026-08-01 — DESIGN §2.3 is authoritative and matches.*
...
- [ ] Detecting overlap with `body_entered` misses bodies that are already inside the area. Everything that must affect a resting or re-entering body re-scans overlaps each tick with a per-player re-trigger gap (`TerrainElement`, both Mason blocks, `Player.ContactSense`).
- [ ] A "is there room above me" shape query whose box sits flush on the ground reports **that ground** as a blocker. Inset it (Siku's headroom probe lifts 4px, or the ability refuses everywhere).
- [ ] A re-trigger gap must outlive the effect it gates. Mason's Keystone re-froze a body the frame its stun ended because the gap (0.6) was shorter than the freeze (0.65); derive the gap from the effect, never hardcode both.
- [ ] Harness debris is a real failure source: abilities parent effects to the **stage**, so a Mason block or a Siku pillar outlives the check that made it, and `MatchStage._respawning` fires a body reset 0.6 s after any elimination. Clear both between checks (`_clear_effects()` / `_reset_bodies()`), and never park a test body on a stage spring.
- [ ] Spawning a solid where bodies stand needs **collision exceptions until they physically leave it** — launching them out first is not enough (a launch moves ~11px/frame, a column is tens of px, and depenetration resolves the surviving overlap the shortest way, usually downward). See `IcePillar.ignore_until_clear`. And when testing a launch, assert **position over time**, not velocity — velocity was correct all through the bug this rule comes from.
